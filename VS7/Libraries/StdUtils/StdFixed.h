...
public:
	...
	{
		m_lFixed = Convert((double) fltVal);
		return *this;
	}

	CStdFixed operator=(double dblVal)
	{
		m_lFixed = Convert(dblVal);
		return *this;
	}
	...
	CStdFixed operator+(const CStdFixed &fxB)
	{
		...
		if(lVal & m_lAddTestMask)
			fxC.m_lFixed = m_lAddNegMask | lVal;
		else
			fxC.m_lFixed = m_lAddPosMask & lVal;
	...
	CStdFixed operator+=(const CStdFixed &fxB)
	{
		...
		//other values are 0.
		if(lVal & m_lAddTestMask)
			this->m_lFixed = m_lAddNegMask | lVal;
		else
			this->m_lFixed = m_lAddPosMask & lVal;
