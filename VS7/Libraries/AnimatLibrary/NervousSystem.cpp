...
Node *NervousSystem::FindNode(string strModuleName, long lNodeID, BOOL bThrowError)
{
	try
	{
		NeuralModule *lpModule = FindNeuralModule(strModuleName);
		return lpModule->FindNode(lNodeID);
	}
	catch(CStdErrorInfo oError)
	{
		if(bThrowError)
...
void NervousSystem::Reset(Simulator *lpSim, Organism *lpOrganism)
{
	NeuralModule *lpModule = NULL;
	CStdPtrMap<string, NeuralModule>::iterator oPos;
	for(oPos=m_aryNeuralModules.begin(); oPos!=m_aryNeuralModules.end(); ++oPos)
	{
		lpModule = oPos->second;
		lpModule->Reset(lpSim, lpOrganism);
	}
}
...
void NervousSystem::Load(Simulator *lpSim, Structure *lpStructure, string strProjectPath, string strNeuralFile)
{
	...
	oXml.Load(AnimatLibrary::GetFilePath(strProjectPath, strNeuralFile));
