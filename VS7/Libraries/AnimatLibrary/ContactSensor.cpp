...
void ContactSensor::Load(Simulator *lpSim, Structure *lpStructure, CStdXml &oXml)
{
	...
	m_lpFieldGain = dynamic_cast<AnimatLibrary::Gains::Gain *>(lpSim->CreateObject(strModuleName, "Gain", strType));
	if(!m_lpFieldGain)
		THROW_TEXT_ERROR(Al_Err_lConvertingClassToType, Al_Err_strConvertingClassToType, "FieldGain");

	m_lpFieldGain->LoadFull(oXml);
	...
	m_lpCurrentGain = dynamic_cast<AnimatLibrary::Gains::Gain *>(lpSim->CreateObject(strModuleName, "Gain", strType));
	if(!m_lpCurrentGain)
		THROW_TEXT_ERROR(Al_Err_lConvertingClassToType, Al_Err_strConvertingClassToType, "FieldGain");

	m_lpCurrentGain->LoadFull(oXml);

	if(m_lpCurrentGain->UseLimits())
		m_fltMaxForce = m_lpCurrentGain->UpperLimit();

	m_fltReceptiveFieldDistance = AnimatLibrary::LoadScaledNumber(oXml, "ReceptiveFieldDistance", FALSE, m_fltReceptiveFieldDistance);
