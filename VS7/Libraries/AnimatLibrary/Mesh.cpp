...
void Mesh::Load(Simulator *lpSim, Structure *lpStructure, CStdXml &oXml)
{
	...
	string strExtension = Std_FileExtension(m_strGraphicsMesh);
	if(!strExtension.length())
		THROW_PARAM_ERROR(Al_Err_lGraphicsMeshNoExtension, Al_Err_strGraphicsMeshNoExtension, "Body", m_strName);

	m_strGraphicsMesh = Std_Replace(m_strGraphicsMesh, strExtension, ".ase");
