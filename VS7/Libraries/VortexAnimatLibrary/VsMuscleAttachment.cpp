// VsMuscleAttachment.cpp: implementation of the VsMuscleAttachment class.
//
//////////////////////////////////////////////////////////////////////
...
VsMuscleAttachment::VsMuscleAttachment()
{
	m_lLastUpdateTime = -1;
	m_lpGraphic = NULL;
}
...
{Std_TraceMsg(0, "Caught Error in desctructor of VsMuscleAttachment\r\n", "", -1, FALSE, TRUE);}
}

CStdFPoint VsMuscleAttachment::CalculatePosition(Simulator *lpSim)
{
	UpdateAttachmentPointPosition(lpSim);
	return m_oAbsPosition;
}

void VsMuscleAttachment::UpdateAttachmentPointPosition(Simulator *lpSim)
{
	...
	if(m_lLastUpdateTime < lpSim->TimeSlice() )
	{
		...
		m_fltReportX = m_oAbsPosition.x * lpSim->DistanceUnits(); 
		m_fltReportY = m_oAbsPosition.y * lpSim->DistanceUnits(); 
		m_fltReportZ = m_oAbsPosition.z * lpSim->DistanceUnits(); 

		m_lLastUpdateTime = lpSim->TimeSlice();
	}
}

void VsMuscleAttachment::CreateParts(Simulator *lpSim, Structure *lpStructure)
{
	TRACE_DEBUG("Statring Parts Creation for " + m_strName);
	TRACE_DETAIL_NS("PartType: MuscleAttachment");
...
void VsMuscleAttachment::StepSimulation(Simulator *lpSim, Structure *lpStructure)
{
	UpdateAttachmentPointPosition(lpSim);
}
