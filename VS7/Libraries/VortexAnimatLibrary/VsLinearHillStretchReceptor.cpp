...
void VsLinearHillStretchReceptor::DrawMuscle(Simulator *lpSim, BOOL bInitDraw)
{
...
void VsLinearHillStretchReceptor::CalculateForceVector(Simulator *lpSim, MuscleAttachment *lpPrim, MuscleAttachment *lpSec, float fltTension, CStdFPoint &oPrimPos, CStdFPoint &oSecPos, CStdFPoint &oPrimForce)
{
...
void VsLinearHillStretchReceptor::StepSimulation(Simulator *lpSim, Structure *lpStructure)
{
	if(m_bEnabled)
	{
		...
		if(m_fltTension > 1e-5 && m_bApplyTension)
		{
			int iCount = m_aryAttachmentPoints.GetSize();
			MuscleAttachment *lpAttach1 = m_aryAttachmentPoints[0], *lpAttach2 = NULL;
