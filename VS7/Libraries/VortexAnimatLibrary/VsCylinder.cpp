...
CStdFPoint VsCylinder::GetCurrentPosition()
{
...
void VsCylinder::CreateParts(Simulator *lpSim, Structure *lpStructure)
{
	...
	TRACE_DETAIL_NS("PartType: Cylinder");
	...
	if(m_bIsVisible)
	  m_lpGraphic = RGraphicCylinderCreate(lpVsSim->RenderContext(), m_fltRadius, m_fltHeight, m_aryColor, MdtBodyGetTransformPtr(m_lpBodyID));

	//Lets get the volume and areas
	m_fltVolume = 2*PI*m_fltRadius*m_fltRadius*m_fltHeight;
	m_fltXArea = 2*m_fltRadius*m_fltHeight;
	m_fltYArea = 2*m_fltRadius*m_fltHeight;
	m_fltZArea = 2*PI*m_fltRadius*m_fltRadius;
...
void VsCylinder::ResetNode(Simulator *lpSim, Structure *lpStructure)
...
void VsCylinder::EnableCollision(RigidBody *lpBody)
{VsRigidBody::EnableCollision(lpBody);}

void VsCylinder::DisableCollision(RigidBody *lpBody)
...
void VsCylinder::AddForce(Simulator *lpSim, float fltPx, float fltPy, float fltPz, float fltFx, float fltFy, float fltFz)
{VsRigidBody::AddBodyForce(lpSim, fltPx, fltPy, fltPz, fltFx, fltFy, fltFz);}

void VsCylinder::AddTorque(Simulator *lpSim, float fltTx, float fltTy, float fltTz)
