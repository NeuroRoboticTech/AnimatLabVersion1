...
namespace Crownwood.Magic.Docking
{
	...
	public class DockingManager
	{
		...
		public delegate void ContentClosingHandler(Content c, CancelEventArgs cea);
		...
		public event ContentClosingHandler ContentClosing;
		...
		//I added this mthod to dispose of all of the control objects
		//for each content. If you do not do this then any controls that
		//explicitly need to be disposed will go Kabooom!
		public void Dispose()
		{
			int iCount = _contents.Count;

			for(int iIndex=0; iIndex<iCount; iIndex++)
			{
				Content c = _contents[iIndex];
				c.Control.Dispose();
			}
		}
		...
		public void AutoHideWindow(Crownwood.Magic.Docking.WindowContent wndContent)
		{
			foreach(WindowDetail wd in wndContent.WindowDetails)
			{
				if(wd is Crownwood.Magic.Docking.WindowDetailCaption)
				{
					Crownwood.Magic.Docking.WindowDetailCaption wdc = (Crownwood.Magic.Docking.WindowDetailCaption) wd;
		...
		{
			foreach(Content ctDock in this.Contents)
			{
				if(ctDock.AutoHidePanel != null)
					ctDock.AutoHidePanel.RemoveContent(ctDock);

				if(ctDock.Control != null)
					ctDock.Control.Dispose();
		...
		public void SaveConfigToXml(ref XmlTextWriter xmlOut)
		{
			...
			for(int index=count-1; index>=0; index--)
			{
				...
				//DWC I removed this because it was causing the system to explode 
				//whenever one of the forms was autohidden.
...
//				ShowContent(c);
		...
		public void LoadConfigFromStream(Stream stream)
		{
			...
			LoadConfigFromXml(ref xmlIn);

			xmlIn.Close();			
		}

		protected void EnsureContentVisible(Content c)
		{
			//Lets check to make sure this content will be visible on the screen.
			//If it is not then lets move it to a visible spot.
			if(!c.Docked && ((c.DisplayLocation.X < 0) || (c.DisplayLocation.Y < 0) || (c.DisplayLocation.X >= this.Container.Size.Width) || (c.DisplayLocation.Y >= this.Container.Size.Height)))
			{
				Crownwood.Magic.Docking.RestoreContentFloatingAffinity floatRestore = (Crownwood.Magic.Docking.RestoreContentFloatingAffinity) c.FloatingRestore;

				if(c.DisplayLocation.X < 0)
				{
					c.DisplayLocation = new System.Drawing.Point(0, c.DisplayLocation.Y);
					floatRestore._location = new Point(c.DisplayLocation.X, c.DisplayLocation.Y);
				}

				if(c.DisplayLocation.Y < 0)
				{
					c.DisplayLocation = new System.Drawing.Point(c.DisplayLocation.X, 0);
					floatRestore._location = new Point(c.DisplayLocation.X, c.DisplayLocation.Y);
				}

				if(c.DisplayLocation.X >= this.Container.Size.Width)
				{
					c.DisplayLocation = new System.Drawing.Point((this.Container.Size.Width - c.DisplaySize.Width), c.DisplayLocation.Y);
					floatRestore._location = new Point(c.DisplayLocation.X, c.DisplayLocation.Y);
				}

				if(c.DisplayLocation.Y >= this.Container.Size.Height)
				{
					c.DisplayLocation = new System.Drawing.Point(c.DisplayLocation.X, (this.Container.Size.Height - c.DisplaySize.Height));
					floatRestore._location = new Point(c.DisplayLocation.X, c.DisplayLocation.Y);
				}
			}
		}

		public void LoadConfigFromXml(ref XmlTextReader xmlIn)
		{
			...
			do
			{
				...
				if (xmlIn.Name == "Content")
				{
					...
					catch(System.Exception ex)
					{
						...
						//Lets eat this error and simply not add this content in if we hit a problem.
					}
				}
				else
				{
		...
		public virtual bool OnContentClosing(Content c)
		{
			try
			{
				CancelEventArgs cea = new CancelEventArgs();
			...
			try
			{
				...
				if (wdc != null)
				{
					...
					if (wct != null)
					{
						...
						if (c != null)
						{
							// Only close the window if the OnContentClosing says we can.
							if (!OnContentClosing(c))
							{
								//I changed this so that when you close a content window it 
								//totally removes it and does not keep it in the contents collection.
								//This was causing some controls to die when closing down. 
								//I also call the dispose method before hand to make sure that the
								//inner control gets shut down correctly before removing the content.
								c.Control.Dispose();
								_contents.Remove(c);
							}
						}
