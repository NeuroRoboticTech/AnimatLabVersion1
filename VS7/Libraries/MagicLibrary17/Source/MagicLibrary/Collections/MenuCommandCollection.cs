...
namespace Crownwood.Magic.Collections
{
    public class MenuCommandCollection : CollectionWithEvents
    {
        ...
        public bool VisibleItems()
        {
			...
			public MenuCommand FindMenuCommand(string strTag)
			{return FindMenuCommand(strTag, true);}

			public MenuCommand FindMenuCommand(string strTag, bool bThrowException)
			{
				MenuCommand mcFound;

				foreach(MenuCommand mc in base.List)
				{
					if(mc.Tag != null &&  mc.Tag.GetType() == strTag.GetType())
					{
						string strMcTag = (string) mc.Tag;
						strMcTag = strMcTag.ToUpper().Trim();
						strTag = strTag.ToUpper().Trim();

						if (strMcTag == strTag)
							return mc;
					}

					mcFound = mc.MenuCommands.FindMenuCommand(strTag, false);
					if(mcFound != null)
						return mcFound;
				}

				if(bThrowException)
					throw new Exception("The menu command with the tag '" + strTag + "' was not found in the menu control.");
