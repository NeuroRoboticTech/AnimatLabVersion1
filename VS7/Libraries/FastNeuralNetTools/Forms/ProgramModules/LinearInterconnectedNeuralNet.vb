...
#End Region
        ...
        Protected Overrides Sub OnLoad(ByVal e As System.EventArgs)
            Try
                ...
                Dim doOrganism As AnimatTools.DataObjects.Physical.Organism
                For Each deEntry As DictionaryEntry In Util.Environment.Organisms
                    doOrganism = DirectCast(deEntry.Value, AnimatTools.DataObjects.Physical.Organism)
        ...
        Protected Sub GenerateNetwork()
            ...
            Dim doOrganism As AnimatTools.DataObjects.Physical.Organism = DirectCast(cboOrganisms.SelectedItem, AnimatTools.DataObjects.Physical.Organism)
            ...
            Dim doDiagram As AnimatTools.Forms.Behavior.Diagram = doOrganism.BehaviorEditor.AddDiagram("LicensedAnimatTools.dll", "LicensedAnimatTools.Forms.Behavior.AddFlowDiagram", Nothing, txtDiagramName.Text)

            Dim fltThetaDiff As Single = CSng((2 * Math.PI) / m_iNumNeurons)
            Dim fltTheta As Single = 0
            Dim aryNeurons As New AnimatTools.Collections.Nodes(Nothing)
            ...
            Dim doNeuron As AnimatTools.DataObjects.Behavior.Node
            ...
            For iNeuron As Integer = 1 To m_iNumNeurons
                doNeuron = DirectCast(doTemplateNeuron.Clone(doTemplateNeuron.Parent, False, Nothing), AnimatTools.DataObjects.Behavior.Node)
            ...
            doTemplateSynapse.Jump = AnimatTools.DataObjects.Behavior.Link.enumJump.None
            Dim fltFij As Single

            Dim doNeuronI As AnimatTools.DataObjects.Behavior.Node
            Dim doNeuronJ As AnimatTools.DataObjects.Behavior.Node
            Dim doSynapse As DataObjects.Behavior.Synapses.Normal
            Dim doLink As AnimatTools.DataObjects.Behavior.Link
            Dim doGain As New AnimatTools.DataObjects.Gains.Bell(Nothing)
            ...
            For iI As Integer = 1 To m_iNumNeurons
                doNeuronI = DirectCast(aryNeurons(iI - 1), AnimatTools.DataObjects.Behavior.Node)

                For iJ As Integer = 1 To m_iNumNeurons
                    If iJ <> iI Then
                        fltFij = CalculateWeight(doGain, iI, iJ)
                        doNeuronJ = DirectCast(aryNeurons(iJ - 1), AnimatTools.DataObjects.Behavior.Node)

                        doSynapse = DirectCast(doTemplateSynapse.Clone(doTemplateSynapse.Parent, False, Nothing), DataObjects.Behavior.Synapses.Normal)
                        doLink = doSynapse
                        doSynapse.Weight = New AnimatTools.Framework.ScaledNumber(doSynapse, "Weight", fltFij, AnimatTools.Framework.ScaledNumber.enumNumericScale.nano, "Amps", "A")
        ...
        Public Function CalculateWeight(ByVal doGain As AnimatTools.DataObjects.Gain, ByVal iI As Integer, ByVal iJ As Integer) As Single
        ...
        Protected Sub ModifyNetwork()
            ...
            Dim doDiagram As AnimatTools.Forms.Behavior.Diagram = doOrganism.BehaviorEditor.FindDiagramByName(txtDiagramName.Text)
            ...
            Dim doOrigin As AnimatTools.DataObjects.Behavior.Node
            Dim doDestination As AnimatTools.DataObjects.Behavior.Node
            Dim doLink As AnimatTools.DataObjects.Behavior.Link
            Dim doSynapse As DataObjects.Behavior.Synapses.Normal
            ...
            For Each deEntry As DictionaryEntry In doDiagram.Nodes
                doOrigin = DirectCast(deEntry.Value, AnimatTools.DataObjects.Behavior.Node)

                If TypeOf doOrigin Is AnimatTools.DataObjects.Behavior.Nodes.Neuron AndAlso IsNumeric(doOrigin.Text) Then

                    iI = CInt(doOrigin.Text)

                    For Each doLinkEntry As DictionaryEntry In doOrigin.OutLinks
                        doLink = DirectCast(doLinkEntry.Value, AnimatTools.DataObjects.Behavior.Link)

                        If Not doLink.ActualDestination Is Nothing AndAlso TypeOf doLink.ActualDestination Is AnimatTools.DataObjects.Behavior.Nodes.Neuron AndAlso IsNumeric(doLink.ActualDestination.Text) Then
                            doDestination = DirectCast(doLink.ActualDestination, AnimatTools.DataObjects.Behavior.Node)
