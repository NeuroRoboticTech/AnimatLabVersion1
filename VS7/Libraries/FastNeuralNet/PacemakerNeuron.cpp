...
float PacemakerNeuron::CalculateIntrinsicCurrent(FastNeuralModule *lpModule, float fltInputCurrent)
{
...
void PacemakerNeuron::StepSimulation(Simulator *lpSim, Organism *lpOrganism, FastNeuralModule *lpModule, unsigned char iXPos, unsigned char iYPos, unsigned char iZPos)
{
	//float fltTime = lpSim->Time();
	//if(m_strName == "U Obstacle Mem" && fltTime > 3.672)
	//	fltTime=fltTime;

	Neuron::StepSimulation(lpSim, lpOrganism, lpModule, iXPos, iYPos, iZPos);
...
void PacemakerNeuron::Save(Simulator *lpSim, Structure *lpStructure, CStdXml &oXml)
{
	...
	oXml.AddChildElement("Vssm", m_fltVssm);
	oXml.AddChildElement("Mtl", m_fltMtl);
	oXml.AddChildElement("Btl", m_fltBtl);
	oXml.AddChildElement("Th", m_fltTh);
}
